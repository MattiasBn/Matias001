...
function normalizeUser(rawUser: User): User {
  return {
    ...
    email: rawUser.email,
    role: rawUser.role,
    telefone: rawUser.telefone ?? null,
    confirmar: rawUser.confirmar,
    password: rawUser.password ?? null,
    photo: rawUser.photo ?? null,
    google_id: rawUser.google_id ?? undefined,
  };
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchLoggedUser = useCallback(async () => {
    ...
    try {
      ...
      if (!normalizedUser.telefone || !normalizedUser.password) {
  ...
  return (
    <AuthContext.Provider value={{ user, isAuthenticated: !!user, login, logout, loading, fetchLoggedUser, loginWithGoogle, refreshUser }}>
      {children}
